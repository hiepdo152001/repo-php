	...
	class SearchController {
		...
		public function ajaxSearch(){
			...
			foreach($data as $rows){
				$strResult = $strResult."<li><img src='assets/upload/products/{$rows->photo}'><a href='index.php?controller=products&action=detail&id={$rows->id}'>{$rows->name}</a><a>{$rows->price}</a>&nbsp</li>";
			}
			echo $strResult;
		}
